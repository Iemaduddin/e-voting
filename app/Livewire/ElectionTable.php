...
final class ElectionTable extends PowerGridComponent
{
    ...
    #[\Livewire\Attributes\On('extend')]
    public function extend($rowId): void
    {
        $this->dispatch('openExtendModal', electionId: $rowId);
    }
    ...
    public function actions(Election $row): array
    {
        ...
        // Tambahkan tombol perpanjang hanya untuk pemilihan yang published
        if ($row->status === 'published') {
            $actions[] = Button::add('extend')
                ->slot('<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>')
                ->id()
                ->class('inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-yellow-600 rounded-lg hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500')
                ->tooltip('Perpanjang Waktu')
                ->dispatch('extend', ['rowId' => $row->id]);
        }
