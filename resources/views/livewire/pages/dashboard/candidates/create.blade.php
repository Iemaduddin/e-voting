...
new #[
    Layout('layouts.dashboard', [
        'subtitle' => 'Tambah Kandidat',
        'pageTitle' => 'Tambah Kandidat',
        'pageDescription' => 'Form untuk menambahkan kandidat baru ke dalam sistem e-voting',
    ]),
]
class extends Component {
    ...
    public function mount($electionId)
    {
        ...
        if ($this->organizationId) {
            ...
            // Get user_ids of members who are already registered as candidates in this election
            $this->registeredUserIds = Candidate::where('election_id', $electionId)
                ->get()
    ...
    public function removeMisi($index)
    {
        ...
        $this->misi = array_values($this->misi); // Re-index array
    }

    public function refreshRegisteredUsers()
    {
        // Refresh the list of registered user IDs
    ...
    public function updated($propertyName)
    {
        ...
        if (in_array($propertyName, ['ketua_id', 'wakil_id', 'visi', 'misi', 'ketua_cv', 'ketua_photo', 'wakil_cv', 'wakil_photo', 'link']) && empty($value)) {
            return;
        }
    ...
    public function getRules()
    {
        ...
        // Conditional validation: if wakil is selected, require wakil photo and cv
        if ($this->wakil_id) {
            $rules['wakil_photo'] = 'required|image|mimes:jpeg,png,jpg,gif|max:2048';
            $rules['wakil_cv'] = 'required|file|mimes:pdf|max:2048';
        } else {
            $rules['wakil_photo'] = 'nullable|image|mimes:jpeg,png,jpg,gif|max:2048';
            $rules['wakil_cv'] = 'nullable|file|mimes:pdf|max:2048';
        }
    ...
    public function getMessages()
    {
        return [
            ...
            'wakil_cv.required' => 'CV Wakil wajib diisi jika memilih wakil',
            ...
            'wakil_photo.required' => 'Foto Wakil wajib diisi jika memilih wakil',
    ...
    public function createCandidate()
    {
        try {
            ...
            // Handle file uploads for Wakil (if exists)
            $wakilCvPath = null;
            $wakilPhotoPath = null;
            ...
            // Prepare JSON data for cv and photo
            $cvData = json_encode([
                'ketua' => $ketuaCvPath,
                'wakil' => $wakilCvPath,
            ]);
            ...
            if ($this->redirectToIndex) {
                ...
                // Reset form for creating another candidate
                $this->reset(['ketua_id', 'wakil_id', 'visi', 'misi', 'ketua_cv', 'ketua_photo', 'wakil_cv', 'wakil_photo', 'link']);
                $this->misi = [''];
                $this->redirectToIndex = true; // Reset to default

                // Refresh registered users list to include newly added candidate
                $this->refreshRegisteredUsers();

                session()->flash('success', 'Kandidat berhasil ditambahkan. Silakan tambah kandidat lain.');
                return;
            }
        } catch (\Illuminate\Validation\ValidationException $e) {
            // Validation error - let Livewire handle it
            throw $e;
        } catch (\Exception $e) {
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form wire:submit="createCandidate" class="space-y-6">
                ...
                @if ($wakil_id)
                    <div class="mb-4">
                        <p class="text-sm text-amber-600 flex items-start gap-2">
                            ...
                            <span>Foto dan CV wakil wajib diisi jika memilih wakil</span>
                        </p>
                    </div>
                @endif
            </div>

            <!-- Layout Conditional: 2 Kolom jika ada Wakil, 1 Kolom jika tidak -->
            <!-- Jika ADA Wakil: Grid 2x2 (Foto Ketua | Foto Wakil / CV Ketua | CV Wakil) -->
            @if ($wakil_id)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <x-image-upload name="wakil_photo" label="Foto Wakil" wire:model="wakil_photo"
                            ...
                            helperText="Format: JPG, PNG. Maksimal 2MB. Wajib jika memilih wakil" required />
                        @error('wakil_photo')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- CV Ketua -->
                    <div>
                        <x-input-label for="ketua_cv" value="CV Ketua" class="text-gray-700 font-semibold" required />
                    ...
                    <div>
                        ...
                        <input type="file" id="wakil_cv" wire:model="wakil_cv" accept=".pdf"
                            class="mt-2 w-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm">
                        <p class="mt-1 text-sm text-gray-500">Format: PDF. Maksimal 2MB. Wajib jika memilih wakil</p>
                        @error('wakil_cv')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
            ...
            @else
                <!-- Jika TIDAK ADA Wakil: 1 Kolom (Foto dan CV Ketua vertikal) -->
                <div class="space-y-6">
                    <!-- Foto Ketua -->
                    <div>
                        <x-image-upload name="ketua_photo" label="Foto Ketua" wire:model="ketua_photo"
            ...
            <div class="flex items-center justify-between gap-3 pt-6 border-t">
                ...
                <div class="flex gap-3">
                    <button type="button" wire:click="createAndStay" wire:loading.attr="disabled"
                        wire:target="createAndStay,createCandidate"
                    ...
                    <button type="submit" wire:loading.attr="disabled" wire:target="createCandidate,createAndStay"
                        ...
                        <span wire:loading wire:target="createCandidate">
                            <svg class="animate-spin h-5 w-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10"
                                    stroke="currentColor" stroke-width="4"></circle>
                        ...
                        <span wire:loading.remove wire:target="createCandidate">Tambah Kandidat</span>
                        <span wire:loading wire:target="createCandidate">Menyimpan...</span>
                    </button>
                </div>
            </div>
        </form>
