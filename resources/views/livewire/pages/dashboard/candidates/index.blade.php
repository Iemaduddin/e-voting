<div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4 sm:mb-6">
    <h3 class="text-lg sm:text-xl font-bold text-gray-900">Manajemen Kandidat</h3>
    <a href="{{ route('candidates.create', ['electionId' => $election->id]) }}" wire:navigate
        class="inline-flex items-center justify-center gap-2 px-3 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors duration-200 whitespace-nowrap">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th scope="col"
                            class="px-3 sm:px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            No
                        </th>
                        <th scope="col"
                            class="px-3 sm:px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <th scope="col"
                            class="hidden lg:table-cell px-3 sm:px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Visi
                        </th>
                        <th scope="col"
                            class="px-3 sm:px-4 md:px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Aksi
                        </th>
                    </tr>
                </thead>
                                ...
                                <div class="flex items-center gap-1.5 sm:gap-2">
                                    @if ($ketuaPhoto)
                                        <img src="{{ asset('storage/' . $ketuaPhoto) }}" alt="Foto Ketua"
                                            class="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover border-2 border-blue-500">
                                    @else
                                        <div
                                            class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-blue-500 flex items-center justify-center text-white font-bold text-xs sm:text-sm">
                                            K
                                        </div>
                                    @endif
                                    ...
                                    @if ($candidate->wakil && $wakilPhoto)
                                        <img src="{{ asset('storage/' . $wakilPhoto) }}" alt="Foto Wakil"
                                            class="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover border-2 border-cyan-500">
                                    @endif
                                </div>
                            </td>
                            <td class="px-3 sm:px-4 md:px-6 py-4 whitespace-nowrap">
                                <div class="text-xs sm:text-sm font-medium text-gray-900">
                                    {{ $candidate->ketua->name ?? 'N/A' }}</div>
                                <div class="text-xs text-gray-500 hidden sm:block">{{ $candidate->ketua->email ?? '-' }}
                                </div>
                            </td>
                            <td class="px-3 sm:px-4 md:px-6 py-4 whitespace-nowrap">
                                @if ($candidate->wakil)
                                    <div class="text-xs sm:text-sm font-medium text-gray-900">
                                        {{ $candidate->wakil->name }}</div>
                                    <div class="text-xs text-gray-500 hidden sm:block">
                                        {{ $candidate->wakil->email ?? '-' }}</div>
                                @else
                                    <span class="text-xs sm:text-sm text-gray-400 italic">Tidak ada wakil</span>
                                @endif
                            </td>
                            <td class="hidden lg:table-cell px-3 sm:px-4 md:px-6 py-4">
